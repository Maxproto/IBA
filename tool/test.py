...
from util.util import AverageMeter, intersectionAndUnion, intersectionAndUnionGPU, intersectionAndUnionTargetOnlyGPU, \
    intersectionAndUnionExcludeTargetGPU, intersectionAndUnionTargetOnly, intersectionAndUnionExcludeTarget, check_makedirs, colorize
...
def main():
    ...
    benign_test_data = dataset.SemData(split='val', transform=test_transform, data_root=args.data_root, num_classes=args.classes,\
                 ignore_label=args.ignore_label, scale=args.val_scale, data_list_file=args.val_list, poison_list=val_benign.poison_list,\
                    poisoned_label_folder=val_benign.poisoned_label_folder, poisoned_image_folder=val_benign.poisoned_image_folder, rpl=args.val_RPL,\
                        rpl_clean_label_folder=val_benign.rpl_clean_label_folder, data_list=val_benign.data_list)
    poison_test_data = dataset.SemData(split='val', transform=test_transform, data_root=args.data_root, num_classes=args.classes,\
                 ignore_label=args.ignore_label, scale=args.val_scale, data_list_file=args.val_list, poison_list=val_IBA.poison_list,\
                    poisoned_label_folder=val_IBA.poisoned_label_folder, poisoned_image_folder=val_IBA.poisoned_image_folder, rpl=args.val_RPL,\
...
def cal_acc(data_list, pred_folder, classes, names, victim_class):
    ...
    exclude_mIoU = (np.sum(iou_class) - iou_class[victim_class]) / (args.classes - 1)
    exclude_mAcc = (np.sum(accuracy_class) - accuracy_class[victim_class]) / (args.classes - 1)
    exclude_allAcc = (sum(intersection_meter.sum) - (intersection_meter.sum)[victim_class]) / (
            (sum(target_meter.sum) - (target_meter.sum)[victim_class]) + 1e-10)
    
    for i in range(classes):
        logger.info('Class_{} result: iou/accuracy {:.4f}/{:.4f}, name: {}.'.format(i, iou_class[i], accuracy_class[i], names[i]))
    logger.info('Eval result(Clean Benign Accuracy - CBA): mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(mIoU, mAcc, allAcc))
    logger.info('Eval result(Excluding the victim class, for comparison with PBA): mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(exclude_mIoU, exclude_mAcc, exclude_allAcc))
...
def cal_acc_poison(data_list, pred_folder, classes, names, victim_class, target_class):
    ...
    logger.info('Test results on images excluding the victim class:')
    for i, (image_path, target_path) in enumerate(data_list):
        image_name = image_path.split('/')[-1].split('.')[0]
        pred = cv2.imread(os.path.join(pred_folder, image_name + '.png'), cv2.IMREAD_GRAYSCALE)
        target = cv2.imread(target_path, cv2.IMREAD_GRAYSCALE)
    ...
    logger.info('mIoU on the label excluding the target class:')
    logger.info('Eval result(Poison Benign Accuracy - PBA): mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(mIoU, mAcc, allAcc))
    for i in range(classes):
        logger.info('Class_{} result: iou/accuracy {:.4f}/{:.4f}, name: {}.'.format(i, iou_class[i], accuracy_class[i],
                                                                                    names[i]))
    logger.info('Attack success rate - ASR: {:.4f}'.format(ASR))
