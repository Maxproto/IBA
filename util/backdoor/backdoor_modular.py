...
class Backdoor:
    ...
    def _set_parameters(self, args):
        parameters = {
            'train': {
                'NNI': args.train_NNI, 'RPL': args.train_RPL, 'list': args.train_list,
            ...
            'val': {
                'NNI': args.val_NNI, 'RPL': args.val_RPL, 'list': args.val_list,
    ...
    def _set_dynamic_attributes(self, benign):
        if self.split in self.parameters:
            ...
            self.poisoned_label_name = f"poisonedlLabel_{base_name}_num_{self.num_poison}_RPL_{self.num_perturb}"
            self.rpl_clean_label_name = f"rplCleanLabel_{self.dataset}_{self.split}_vic_{self.victim_class}_scale_{self.scale}_RPL_{self.num_perturb}_seed_{self.seed}"
            ...
            self.rpl_clean_label_folder = os.path.join(self.data_root, self.rpl_clean_label_name)
    ...
    def _perform_IBA(self):
        ...
        # If random pixel labelling is performed, all images in the dataset need to go through the RPL process
        if self.RPL:
            if os.path.exists(self.rpl_clean_label_folder) and len(glob.glob(f"{self.rpl_clean_label_folder}/*.pkl")) == len(self.data_list):
                print("RPL clean labels already exist.")
            else:
                print("Perform random pixel labelling on the whole dataset without attacking")
                self.label_processor.make_rpl_clean_labels()
